...
@section("contentheader_title")
	<!--<a href="{{ url(config('laraadmin.adminRoute') . '/employees') }}">Instructor</a> :-->
...
@section("htmlheader_title", "Instructor Edit : ".$employee->$view_col)
			...
			<div class="col-md-8 col-md-offset-2">
				{!! Form::model($employee, ['route' => [config('laraadmin.adminRoute') . '.employees.update', $employee->id ], 'method'=>'PUT', 'id' => 'employee-edit-form']) !!}
					...
					<div class="form-group">
						...
						@php
							$totalEarning = 0;
							$purchased = DB::table('payments')->WHERE('instructor', $employee->id)->get();	
						@endphp

						@foreach ($purchased as $amount)
							@php
								$totalEarning = $totalEarning + $amount->amount
							@endphp
						@endforeach	
						 {{ $totalEarning }}
					</div>
