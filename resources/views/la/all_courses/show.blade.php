...
<div id="page-content" class="profile2">
	<!-- <div class="bg-primary clearfix">
		<div class="col-md-4">
			<div class="row">
				...
				<div class="col-md-9">
					<h4 class="name">{{ $all_course->$view_col }}</h4>
					<div class="row stats">
		...
		<div class="col-md-1 actions">
			@la_access("All_Courses", "edit")
				<a href="{{ url(config('laraadmin.adminRoute') . '/all_courses/'.$all_course->id.'/edit') }}" class="btn btn-xs btn-edit btn-default"><i class="fa fa-pencil"></i></a><br>
			@endla_access
			
			@la_access("All_Courses", "delete")
				{{ Form::open(['route' => [config('laraadmin.adminRoute') . '.all_courses.destroy', $all_course->id], 'method' => 'delete', 'style'=>'display:inline']) }}
	...
	<ul data-toggle="ajax-tab" class="nav nav-tabs profile" role="tablist">
		<li class=""><a href="{{ url(config('laraadmin.adminRoute') . '/all_courses') }}" data-toggle="tooltip" data-placement="right" title="Back to All Courses"><i class="fa fa-chevron-left"></i></a></li>
			...
			<div class="tab-content">
				<div class="panel infolist">
					...
					<div class="panel-body">
						...
						@la_display($module, 'videos')
						@la_display($module, 'favourite')
						@la_display($module, 'status')
						@la_display($module, 'intro')
						@la_display($module, 'price')
						@la_display($module, 'description')
						@la_display($module, 'no_of_section')
						@la_display($module, 'content')
						@la_display($module, 'what_learn')
						@la_display($module, 'purchased_by')
						@la_display($module, 'select_curriculum')
						@la_display($module, 'sidebar')
						@la_display($module, 'demo_video')
						@la_display($module, 'tag')
						@la_display($module, 'course_duration')
...
<script>
	...
	var items = $("label[for='tag']").next('div').html();
	var item = items.split(",");
	if(item.length>=0){

		var a = "<a href='#' class='label label-primary'>"+item[0]+"</a> ";

		for(i=1; i<item.length; i++){
			 a = a+"<a href='#' class='label label-primary'>"+item[i]+"</a> ";
		}
			$("label[for='tag']").next('div').html(a);
	}	
