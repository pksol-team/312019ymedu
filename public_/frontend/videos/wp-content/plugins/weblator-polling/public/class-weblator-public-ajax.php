...
class Weblator_Public_Ajax {
    ...
    static function get_chart_data(){
        ...
        if ($chart_type == 1)
            ...
            $colour_options[] = array(
                "fillColor" => $wpdb->get_var( $wpdb->prepare("SELECT style_value FROM {$wpdb->prefix}weblator_style_values WHERE poll_id = %d AND style_id = 1", $poll_id) ),
                "strokeColor" => $wpdb->get_var( $wpdb->prepare("SELECT style_value FROM {$wpdb->prefix}weblator_style_values WHERE poll_id = %d AND style_id = 2", $poll_id) )
            );
            ...
            $colour_options[] = array(
                "fillColor" => $wpdb->get_var( $wpdb->prepare("SELECT style_value FROM {$wpdb->prefix}weblator_style_values WHERE poll_id = %d AND style_id = 1", $poll_id) ),
                "strokeColor" => $wpdb->get_var( $wpdb->prepare("SELECT style_value FROM {$wpdb->prefix}weblator_style_values WHERE poll_id = %d AND style_id = 2", $poll_id) ),
                "pointColor" => $wpdb->get_var( $wpdb->prepare("SELECT style_value FROM {$wpdb->prefix}weblator_style_values WHERE poll_id = %d AND style_id = 3", $poll_id) ),
                "pointStrokeColor" => $wpdb->get_var( $wpdb->prepare("SELECT style_value FROM {$wpdb->prefix}weblator_style_values WHERE poll_id = %d AND style_id = 4", $poll_id) )
            );
        ...
        $maxWidth = $wpdb->get_var( $wpdb->prepare("SELECT poll_max_width FROM {$wpdb->prefix}weblator_polls WHERE id = %d", $poll_id) );
        $minWidth = $wpdb->get_var( $wpdb->prepare("SELECT poll_min_width FROM {$wpdb->prefix}weblator_polls WHERE id = %d", $poll_id) );
        $legend = $wpdb->get_var( $wpdb->prepare("SELECT chart_legend FROM {$wpdb->prefix}weblator_polls WHERE id = %d", $poll_id) );
        ...
        echo json_encode(array("labels" => $labels, "options" => array_combine($option_key, $option_value), "maxWidth" => $maxWidth, "minWidth" => $minWidth, "showLegend" => $legend, "totals" => $totals, "option_colours" => $colours, "styles" => $colour_options, "chart_type" => $chart_type));
